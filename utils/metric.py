...
class BinaryMetrics:
    ...
    def _calculate_overlap_metrics(self, gt, pred):
        gt = torch.concatenate((gt, (torch.sum(gt, dim = 1) > 0)[:, None].to(gt.dtype)), dim = 1)
        pred = torch.concatenate((pred, (torch.sum(pred, dim = 1) > 0)[:, None].to(pred.dtype)), dim = 1)

        tp = torch.sum(pred * gt, dim = (2,3))  # TP
        fp = torch.sum(pred * (~gt), dim = (2,3))  # FP
        fn = torch.sum((~pred) * gt, dim = (2,3))  # FN
        tn = torch.sum((~pred) * (~gt), dim = (2,3))  # TN
        ...
        return pixel_acc, dice, precision, specificity, recall, iou, f1
    ...
    def __call__(self, y_true, y_pred):
        ...
        if self.activation in [None, "none"]:
            ...
            y_pred = (y_pred > 0.5).bool().to(y_pred.device)
        else:
            raise NotImplementedError("Not a supported activation!")

        pixel_acc, dice, precision, specificity, recall, iou, f1 = (
            self._calculate_overlap_metrics(
                y_true.to(y_pred.device, dtype=torch.bool), y_pred
            )
        )
        return [pixel_acc, dice, precision, specificity, recall, iou, f1]

class BinaryMetrics_temp:
    r"""Calculate common metrics in binary cases.
    In binary cases it should be noted that y_pred shape shall be like (N, 1, H, W), or an assertion
    error will be raised.
    Also this calculator provides the function to calculate specificity, also known as true negative
    ...
    def _calculate_overlap_metrics(self, gt, pred):
        gt = (torch.sum(gt, dim = 1)>0).to(torch.int8)
        pred = (torch.sum(pred, dim = 1)>0).to(torch.int8)

        tp = torch.sum(pred * gt, dim = (1, 2))  # TP
        fp = torch.sum(pred * (1 - gt), dim = (1, 2))  # FP
        fn = torch.sum((1 - pred) * gt, dim = (1, 2))  # FN
        tn = torch.sum((1 - pred) * (1 - gt), dim = (1, 2))  # TN
